...
def default_output_path(image_path: Path, output_dir: Path) -> Path:
    ...
    return output_dir / f"{line_no}_{station_name}_clicked.json"
...
def load_annotation(input_path: Path):
    ...
    for node in annotation.get("nodes", []):
        xy = node.get("image_xy")
        if not isinstance(xy, list) or len(xy) != 2:
            skipped_nodes += 1
            continue

        point = {
            "kind": node.get("kind", "hall"),
            "image_xy": (int(xy[0]), int(xy[1])),
        }
        if node.get("floor"):
            point["floor"] = node["floor"]
        if node.get("entrance_no"):
            point["entrance_no"] = node["entrance_no"]

        node_id = node.get("id") or f"node_{len(points) + 1:03d}"
        node_id_to_idx[node_id] = len(points)
        points.append(point)

    edges = []
    skipped_edges = 0
    for edge in annotation.get("edges", []):
        from_idx = node_id_to_idx.get(edge.get("from"))
        to_idx = node_id_to_idx.get(edge.get("to"))
        if from_idx is None or to_idx is None:
            skipped_edges += 1
            continue
        edges.append((from_idx, to_idx))

    return points, edges, skipped_nodes, skipped_edges


def infer_image_path_from_annotation_path(annotation_path: Path) -> Path | None:
    stem = annotation_path.stem
    suffix = "_clicked"
    if not stem.endswith(suffix) or "_" not in stem:
        return None

    base_name = stem[:-len(suffix)]
    line_no, station_name = base_name.split("_", 1)
    graph_tool_dir = annotation_path.parent.parent.parent
    station_dir = graph_tool_dir / line_no
    for extension in (".jpg", ".JPG", ".jpeg", ".JPEG", ".png", ".PNG"):
        candidate = station_dir / f"{station_name}{extension}"
        if candidate.exists():
            return candidate
    return None
...
def main():
    ...
    parser.add_argument(
        "annotation",
        nargs="?",
        help="Optional annotation JSON to load directly, e.g. annotations/clicked/7_숭실대입구_clicked.json.",
    )
    parser.add_argument(
        "--image",
        help="Image path to open, e.g. 7/장승배기.jpg. Optional when --load uses {line}_{station}_clicked.json.",
    )
    parser.add_argument(
        "--output-dir",
        default="annotations/clicked",
        help="Directory for auto-named {line}_{station}_clicked.json annotation output.",
    )
    parser.add_argument("--output", help="Optional explicit output JSON path.")
    parser.add_argument(
        "--load",
        help="Existing annotation JSON to load before editing. Defaults to the output path if it exists.",
    )
    parser.add_argument(
        "--no-load",
        action="store_true",
        help="Start with an empty annotation even when the output path already exists.",
    )
    ...
    requested_load_path = Path(args.load or args.annotation) if args.load or args.annotation else None
    if requested_load_path and not requested_load_path.exists():
        raise FileNotFoundError(f"load annotation not found: {requested_load_path}")

    image_path = Path(args.image) if args.image else None
    if image_path is None and requested_load_path:
        image_path = infer_image_path_from_annotation_path(requested_load_path)
        if image_path:
            print(f"[IMAGE] inferred from annotation path: {image_path}")
    if image_path is None:
        parser.error("--image is required unless --load points to a {line}_{station}_clicked.json file")

    output_path = Path(args.output) if args.output else requested_load_path or default_output_path(image_path, Path(args.output_dir))
    ...
    load_path = None
    if requested_load_path:
        load_path = requested_load_path
    elif not args.no_load and output_path.exists():
        load_path = output_path

    if load_path:
        points, edges, skipped_nodes, skipped_edges = load_annotation(load_path)
    else:
        points, edges, skipped_nodes, skipped_edges = [], [], 0, 0
    ...
    if load_path:
        print(
            f"[LOAD] {load_path} nodes={len(points)}, edges={len(edges)}, "
            f"skipped_nodes={skipped_nodes}, skipped_edges={skipped_edges}"
        )
    ...
    print("[TYPE] 1 hall, 2 stairs_start, 3 stairs_end, 4 elevator, 5 platform, 6 entrance_connection, 7 information, 8 qr")
